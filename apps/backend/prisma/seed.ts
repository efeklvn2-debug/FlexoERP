...
async function main() {
  ...
  if (!tenant) {
    ...
    console.log('Created default tenant: Demo Factory')
  }
  ...
  await runWithTenant(tenant.id, async () => {
    ...
    const rolePerms: Record<Role, string[]> = {
      ...
      VIEWER: [
        'auth:read', 'sales_order:read', 'production:read', 'inventory:read',
        'finance:read', 'settings:read', 'customer:read', 'supplier:read',
    ...
    for (const ic of defaultInkColors) {
      await prisma.inkColor.upsert({
        where: { tenantId_name: { tenantId: tenant!.id, name: ic.name } },
        update: {},
        create: ic as any
      })
    }
